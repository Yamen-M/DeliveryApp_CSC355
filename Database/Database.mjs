...
export default class Database {
  ...
  async #connect() {
    ...
    this.#pool = mysql.createPool({
      host: config.dbHost, // database server address — read from validated config
      port: config.dbPort, // database port — read from validated config
      user: config.dbUser, // database username — read from validated config
      password: config.dbPassword, // database password — read from validated config
      database: config.dbName, // database/schema name — read from validated config
    ...
    try {
      ...
      logger.error(`Runtime Schema Error: ${err}`);
    }
  }
